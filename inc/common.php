...
function the_mb_image($id, $size = -1, $option_name = 'my_options', $limit = 1, $returnArray = false)
{
    return get_mb_image($id, $size, $option_name, $limit, $returnArray);
}
function get_mb_image($id, $size, $option_name = 'my_options', $limit = 1, $returnArray = false)
{
    ...
    return get_template_directory_uri() . '/<?php echo get_stylesheet_directory_uri(); ?>/themefrontend/images/no-image.svg';
}
function get_custom_excerpt($limit = 50)
{
    global $post;
    return empty($post->post_excerpt) ? wlimitExcerpt($post->post_excerpt, $limit) : wlimitExcerpt($post->post_excerpt, $limit);
}
...
{
    if ($urlOnly) return get_template_directory_uri() . '/<?php echo get_stylesheet_directory_uri(); ?>/themefrontend/images/no-image.svg';
    return '<img src="' . get_template_directory_uri() . '/<?php echo get_stylesheet_directory_uri(); ?>/themefrontend/images/no-image.svg" class="' . $class . '" alt="No image" title="No image">';
}
...
function getImageTerm($key = 'term-img', $termId = 0)
{
    ...
    if ($img == "") {
        $img = get_template_directory_uri() . '/<?php echo get_stylesheet_directory_uri(); ?>/themefrontend/images/no-image.svg';
    }
    return $img;
}
